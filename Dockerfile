...
ENV DB_HOST=db
ENV DB_PORT=5432
ENV DB_SOURCE=postgresql://postgres:postgres@${DB_HOST}:${DB_PORT}/simple_bank?sslmode=disable
...
ENTRYPOINT [ "sh", "-c", "/app/wait-for ${DB_HOST}:${DB_PORT} -- /app/migrate -path /app/migration -database ${DB_SOURCE} -verbose up" ]
